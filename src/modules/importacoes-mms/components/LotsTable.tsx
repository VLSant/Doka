      ...
      <table className="mms-lots-table">
        ...
        <thead>
          <tr>
            ...
            <th scope="col">Data operacional</th>
        ...
        <tbody>
          {lots.map((lot) => (
            <tr key={lot.lote_id}>
              ...
              <td>
                {lot.postos.map((posto) => posto.nome).join(", ")}
                {lot.visibilidade_parcial ? " (parcial)" : ""}
              </td>
              ...
              <td>
                ...
                >
                  {lot.status ?? lot.estado_processamento}
                </StatusBadge>
              </td>
              <td data-numeric>
                {lot.total_linhas} ({lot.total_assistencias} assistências / {lot.total_partes}{" "}
                partes)
              </td>
              <td data-numeric>
                {lot.total_erros_pendentes} / {lot.total_alertas}
              </td>
              <td>
                {lot.capacidades.abrir ? (
                  <Link to={`/app/importacoes-mms/${lot.lote_id}`}>Abrir</Link>
                ) : (
                  "-"
                )}
              </td>
